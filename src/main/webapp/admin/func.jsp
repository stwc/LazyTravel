...
<div id="main" class="p-3">
  <div class="mx-3 mb-4 d-flex align-items-start">
    <h3 class="d-inline-block me-3">後臺功能</h3>
...
<script>
    function init() {
        $.ajax({
            url: "<%=request.getContextPath()%>/admin/func.do",
            type: "GET",
            data: {"action": "getall"},
            dataType: "json",
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                data.forEach((item) => {
                    ...
                    let modifyBtn =
                        ...
                        '<button type="submit" class="btn-modify btn">修改</button>' +
                        '<input type="hidden" name="customer_id" value="' + item.funcId + '">' +
                        '<input type="hidden" name="action" value="getOneModify">' +
                        '</form>' +
                        '</td>';

                    tmpArr.push(modifyBtn);
                    tmpArr.push(item.funcId);
                    tmpArr.push(item.funcName);
                    tmpArr.push(item.funcDescr);
                    dataSet.push(tmpArr)
                });
