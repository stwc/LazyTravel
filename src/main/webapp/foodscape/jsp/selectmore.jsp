...
<%
    // 讀取從上一頁面傳遞過來的 foodScapeId
    String foodScapeIdString = request.getParameter("foodScapeId");
    if (foodScapeIdString != null && !foodScapeIdString.isEmpty()) {
        int foodScapeId = Integer.parseInt(foodScapeIdString);

        // 根據 foodScapeId 從資料庫中讀取相應的 FoodScape 物件
        // 假設你有一個 FoodScapeDAO 類別用於資料庫操作
        FoodScapeDAO foodScapeDAO = new FoodScapeDAOImpl();
        FoodScape foodScape = foodScapeDAO.getByPK(foodScapeId);

        // 將 foodScape 物件設定到 request 中，以便在 JSP 中使用
        request.setAttribute("foodScape", foodScape);
        RequestDispatcher dispatcher = request.getRequestDispatcher("/foodscape/jsp/foodScapeDetail.jsp");
        dispatcher.forward(request, response);
    } else {
        // 處理 foodScapeId 未提供的情況
